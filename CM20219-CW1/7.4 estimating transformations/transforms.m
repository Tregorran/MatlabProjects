...
theta = -45;
T_d = [cosd(theta) -sind(theta) 0; sind(theta) cosd(theta) 7; 0 0 1];

% (e) Isotropic scale + rotation + translation.
theta = 45;
T_e = [cosd(theta)/2 -sind(theta)/2 2; sind(theta)/2 cosd(theta)/2 10; 0 0 1];

% (f) Anisotropic scale + rotation + translation.
theta = -135;
T_f = [cosd(theta)*0.4 -sind(theta)*0.8 4; sind(theta)*0.4 cosd(theta)*0.8 14; 0 0 1];
...
scaleX = 0.9;
scaleY = 0.9;
T_g = [scaleX skewX 7.975; skewY scaleY 8.025; 0 0 1];

% (h) Perspective.
T_h = [0.2720,-0.8528,9.8120;-0.2293,-0.2168,5.6490;-0.0191,-0.0912,1];
